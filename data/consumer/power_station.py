...
def monitor_events(poll_interval = 1.5):
    while True:
        ...
        if chain.functions.get_current_seqnum().call() > \
            chain.functions.get_last_settlement().call() + 1:
            settle()
...
def settle():
    ...
    # response = chain.functions.settle(seq_num).call()
    # response = response_formatter % (response[0], response[1], response[2])
    
    # execute the function publicly
    tx_hash = chain.functions.settle().transact({'from': from_account})
    '''
    TODO BEGIN:
    ...
    settle_events = chain.events.SettleEvent().processReceipt(tx_receipt)
    price_events = chain.events.PriceEvent().processReceipt(tx_receipt)
...
if __name__ == '__main__':
    ...
    chain = get_contract(node, 'chain')

    try:
        # register as a power station
        tx_hash = chain.functions.register_power_station().transact({'from': from_account})
    ...
    price_event_filter = chain.events.PriceEvent.createFilter(fromBlock=0)
    settle_event_filter = chain.events.SettleEvent.createFilter(fromBlock=0)
